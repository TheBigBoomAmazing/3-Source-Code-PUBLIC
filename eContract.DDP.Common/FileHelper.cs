...
namespace eContract.DDP.Common
{
    ...
    {
        public static void AddColumn(DataTable dt, int addNum, string[] values)
        {
            int count = dt.Columns.Count;
            for (int i = 0; i < values.Length; i++)
            {
                DataColumn column = null;
                column = new DataColumn(i.ToString(), typeof(string));
        ...
        public static DataTable CSVToDataTable(string fn)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    string input = "";
                    for (int i = 1; (input = reader.ReadLine()) != null; i++)
                    {
                        string[] values = null;
                        values = Regex.Split(input, pattern, RegexOptions.IgnoreCase);
                        int count = dt.Columns.Count;
                        int addNum = values.Length - count;
                        if (addNum > 0)
                        {
                            AddColumn(dt, addNum, values);
                        }
